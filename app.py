...
# COLORES IUBELLO
# ============================================================
AZUL_OSCURO = "#1a3a5c"      # Azul oscuro del logo (Bella)
AZUL_CLARO = "#5B9BD5"       # Azul claro de la U
ROJO_IU = "#C8102E"          # Rojo del fuego
GRIS_TEXTO = "#666666"       # Gris del texto institucional
GRIS_FONDO = "#f4f6f8"       # Fondo gris muy claro
VERDE_GRATIS = "#27ae60"     # Verde para tag GRATIS
BLANCO = "#ffffff"
NEGRO = "#2c3e50"
...
<style>
    ...
    div[data-testid="stLinkButton"] > a {{
        background: {BLANCO};
        color: {AZUL_OSCURO};
        border: 1.5px solid {AZUL_OSCURO};
        border-radius: 6px;
        font-weight: 600;
        font-size: 0.85rem;
    }}
    ...
    .success-box {{
        background: #d4edda;
        border-left: 4px solid {VERDE_GRATIS};
        padding: 16px;
        border-radius: 8px;
        color: #155724;
    }}

    /* Warning Streamlit Cloud */
    .cloud-notice {{
        background: #fff3cd;
...
def cargar_csv_inicial():
    """Carga el CSV desde disco. Si no existe, crea uno vacio."""
...
with st.sidebar:
    ...
    for label, key in menu_opciones.items():
...
# ============================================================
elif st.session_state.pagina == 'agregar':
    ...
    # Aviso importante para Streamlit Cloud
    st.markdown(f"""
    ...
    if df.empty or len(df) == 0:
        ...
        with c1:
            ...
            if not tipo_counts.empty:
                st.bar_chart(tipo_counts, color=AZUL_OSCURO)
            else:
                st.info("Sin datos de tipo.")

        with c2:
            st.subheader("🔓 Por tipo de acceso")
            acc_counts = df[df['acceso'] != '']['acceso'].value_counts()
            if not acc_counts.empty:
                st.bar_chart(acc_counts, color=AZUL_CLARO)
            else:
                st.info("Sin datos de acceso.")

        c3, c4 = st.columns(2)
        with c3:
            st.subheader("🌍 Por pais")
            pais_counts = df[df['pais'] != '']['pais'].value_counts()
            if not pais_counts.empty:
                st.dataframe(pais_counts.reset_index().rename(columns={'index': 'Pais', 'pais': 'Cantidad'}), 
                           use_container_width=True, hide_index=True)
            else:
                st.info("Sin datos de pais.")

        with c4:
            st.subheader("🗣️ Por idioma")
            idi_counts = df[df['idioma'] != '']['idioma'].value_counts()
            if not idi_counts.empty:
                st.dataframe(idi_counts.reset_index().rename(columns={'index': 'Idioma', 'idioma': 'Cantidad'}),
                           use_container_width=True, hide_index=True)
            else:
